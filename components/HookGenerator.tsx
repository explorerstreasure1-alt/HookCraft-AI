...
export default function HookGenerator() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const nextSet: HookSet = {
        ...
        hooks: data.hooks ?? [],
        script: data.script,
        mode: data.mode || "hooks",
      };
      setArchive((current) => [nextSet, ...current].slice(0, 10));
  ...
  async function copyHook(hook: string) {
    try { await navigator.clipboard.writeText(hook); setCopiedHook(hook); window.setTimeout(() => setCopiedHook(""), 1400); }
    catch { setCopiedHook("Copy unavailable"); }
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 text-base leading-8 text-[#fdfbf7]/68">
            {user ? "Powered by Mistral AI. Switch between hook-only and full script mode." : "Guest mode. Sign in to save credits across devices."}
          </p>
          <div className="mt-6 flex items-center gap-4 flex-wrap">
            ...
            {credits !== null && credits <= 1 && (
              <a href="#pricing" className="text-xs font-semibold uppercase tracking-[0.15em] text-[#d4af37] hover:text-[#f0d36b] transition">Buy credits ↓</a>
            )}
          </div>
        </div>
          ...
          <aside className="border border-white/10 bg-[#121214] p-5">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                ...
                <p className="mt-1 text-lg font-semibold text-[#fdfbf7]">Sessions</p>
              </div>
            </div>
            ...
            <div className="border border-white/10 bg-[#1a2332]/82 p-6">
              ...
              <div className="mt-4 flex rounded-full border border-white/10 bg-[#121214] p-1">
                <button
                  ...
                  className={`flex-1 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.15em] transition ${mode === "hooks" ? "bg-[#d4af37] text-[#121214]" : "text-[#fdfbf7]/50"}`}
                >
                  Hooks Only
                </button>
                <button
                  ...
                  className={`flex-1 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.15em] transition ${mode === "script" ? "bg-[#d4af37] text-[#121214]" : "text-[#fdfbf7]/50"}`}
                >
                  Full Script
                </button>
              </div>
              ...
              >
                {loading ? "Generating..." : credits !== null && credits > 0 ? (mode === "script" ? "Generate Full Script" : "Architect 3 Hooks") : "Buy credits to continue"}
              </button>
            </div>
            ...
            <div className="border border-[#d4af37]/24 bg-[#121214] p-6 shadow-[0_0_60px_rgba(212,175,55,0.08)]">
              <div className="mb-5 flex items-start justify-between gap-4 border-b border-white/10 pb-4">
                <div>
                  ...
                  <h3 className="mt-2 text-xl font-semibold tracking-[-0.03em]">{activeSet.topic}</h3>
                </div>
                <span className="shrink-0 border border-white/10 px-3 py-2 text-xs text-[#fdfbf7]/58">{activeSet.platform}</span>
              </div>

              {activeSet.mode === "script" && activeSet.script ? (
                <div className="space-y-5">
                  <div className="border border-[#d4af37]/30 bg-[#1a2332]/40 p-4">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-[#d4af37] mb-2">Hook (first 3 sec)</p>
                    <p className="text-lg leading-7 text-[#fdfbf7]/90">{activeSet.script.hook}</p>
                    <button type="button" onClick={() => copyHook(activeSet.script!.hook)} className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#d4af37] hover:text-[#f0d36b]">{copiedHook === activeSet.script.hook ? "Copied" : "Copy"}</button>
                  </div>
                  <div className="border border-white/10 bg-[#1a2332]/40 p-4">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-[#d4af37] mb-2">Body</p>
                    <p className="text-sm leading-7 text-[#fdfbf7]/74">{activeSet.script.body}</p>
                    <button type="button" onClick={() => copyHook(activeSet.script!.body)} className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#d4af37] hover:text-[#f0d36b]">{copiedHook === activeSet.script.body ? "Copied" : "Copy"}</button>
                  </div>
                  <div className="border border-white/10 bg-[#1a2332]/40 p-4">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-[#d4af37] mb-2">Call to Action</p>
                    <p className="text-lg leading-7 text-[#fdfbf7]/90">{activeSet.script.cta}</p>
                    <button type="button" onClick={() => copyHook(activeSet.script!.cta)} className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#d4af37] hover:text-[#f0d36b]">{copiedHook === activeSet.script.cta ? "Copied" : "Copy"}</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  {activeSet.hooks.map((hook, index) => (
                    <div key={hook} className="group border border-white/10 bg-[#1a2332]/58 p-4 transition hover:border-[#d4af37]/55">
                      ...
                      <button type="button" onClick={() => copyHook(hook)} className="mt-4 text-xs font-semibold uppercase tracking-[0.2em] text-[#d4af37] transition group-hover:text-[#f0d36b]">{copiedHook === hook ? "Copied" : "Copy hook"}</button>
                    </div>
                  ))}
                </div>
              )}
